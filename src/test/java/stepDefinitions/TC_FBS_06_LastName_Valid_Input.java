...
public class TC_FBS_06_LastName_Valid_Input {
	...
	@When("Fill the Last Name field of the form by {string}")
	public void fill_the_last_name_field_of_the_form_by(String row) {
	    
		try {
			int index=Integer.parseInt(row)-1;
			String lName = ExcelReadWrite.getCellData(filepath,"sheet1",index,7);
			CFC.lastName(lName);
	    }
	    catch(Exception e) {
	    	e.getMessage();
	...
	@Then("The last name field should be present and accept the {string} input value")
	public void the_field_should_be_present_and_accept_the_input_value(String row) {
	    
		try {
			int index=Integer.parseInt(row)-1;
			String lName = ExcelReadWrite.getCellData(filepath,"sheet1",index,7);
			boolean exp=CFC.checkLname(lName);
